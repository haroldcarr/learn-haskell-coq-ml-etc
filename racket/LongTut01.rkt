...
(module+ test
  (test-equal (lambda? eb1) #true)   ;; HC was #false
  (test-equal (lambda? eb2) #false)
)

;; 2.2.2 Developing Metafunctions : function on the terms of a specific language.

;; disallow param sequences with repeated variables.
...
(define-metafunction Lambda--
  ...
  [(unique-vars) #true]                            ;; all unique
  [(unique-vars x x_1 ... x x_2 ...) #false]       ;; not unique, do not recurse
  [(unique-vars x x_1 ...) (unique-vars x_1 ...)]) ;; recursion

(define-language Lambda
  (e ::=
     x
     (lambda (x_!_ ...) e) ;; x_!_ ... : x must differ from all other elements of seq
     (e e ...))
  (x ::= variable-not-otherwise-mentioned))
...
;; scope
;; fv : free-variables : specifies which language constructs bind
...
;; make metafunctions generic as possible
;; use `any` (from Redex contracts)
;; `sd` below works for extensions that don’t add binders
(module+ test
  (test-equal (SD? sd1) #true))
...
;; syntactic equivalent of function application
