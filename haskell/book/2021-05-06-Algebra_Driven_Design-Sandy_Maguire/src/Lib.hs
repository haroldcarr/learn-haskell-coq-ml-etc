...
-- terminal constructors: haskell and church (i.e., images)
haskell :: Tile
haskell  = undefined
...
Exercise
 ...
 where the ^ operation means repeated composition (e.g., cw^4 = cw . cw . cw . cw)

Horizontally flipping a clockwise rotation is equivalent to
rotating counterclockwise a horizontal flip.
...
Exercise
 ...
 Derive fact that flipV is its own inverse,
 using any of the other laws we’ve given for our algebra.
...
- to maintain closure, must subdivide square into two rectangular halves,
  then fill each half, stretching tiles to cover space
-}
...
Intuitively, can rewrite an above of besides as a beside of aboves,
so long as we swap the top-right and bottom-left tiles when we do so.
...
special case, rotate one tile as we move through a quad,
creating a sort of swirl effect as in figure 18.
...
- cannot be definitional equality,
  because equation that t = cw (cw (cw (cw t))) is not equal syntactically
- two tiles are equal IFF they render to equal images : equal matrices of pixels

Semantics of tile algebra freely subdivide space,
so images generated by it can have arbitrarily precise levels of detail.
-}
...
e.g, flipV operation moves  bottom row of pixels to top,
specifically, it should reverse order of the rows.
∀ (t :: Tile) (w :: Int) (h :: Int).
...
To put one tile above another,
split height in half, then concatenate rows of top raster with bottom.
...
The clockwise cw operation requires us to rotate our rasterized matrix.
I didn’t know of any built-in function to perform this operation,
so I experimented until I found fmap reverse . transpose which works, though admittedly in
