...
Last Modified : 2013 Nov 03 (Sun) 16:17:47 by carr.
...
In the case of =Maybe=, that means =return x= is the same as =Just x=:
...
can be used to represent none (=[]=) or one or more (=[x, ...]=) results (i.e., non-deterministic).
...
between the different =Monad= class instances.  It is really not
necessary to check for =[]= to "short-circuit" further evaluation,
since any function returning =[]= will effectively "stop" the
pipeline:
