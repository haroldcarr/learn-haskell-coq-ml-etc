...
Last Modified : 2013 Oct 16 (Wed) 09:46:23 by carr.
...
#+BEGIN_SRC haskell
stateExampleNonMonadic i f1 f2 =
    case i of
        (c,s:ss,r) -> case f1 (s,ss,c:r) of
                          (c,s:ss,r) -> f2 (s,ss,c:r)

upper (c,s:ss,r) = (s, ss,        toUpper c:r)
shift (c,s:ss,r) = (s, ss, chr (ord c + 10):r)

t11 = t "s1"
      [stateExampleNonMonadic ('h',"ello world", "") upper shift
      ]
      ('o'," world","vlEh")
#+END_SRC
...
stateExampleNonMonadic' :: State (Char, String, String) String -> State (Char, String, String) String
stateExampleNonMonadic' i = i
-- stateExampleNonMonadic' $ return "STRING"
#+END_SRC
...
data ProgramState = PS [Int]
foo :: Int -> String -> State (Int, ProgramState)
foo int string = return (int, PS [int])
{-
-- State (Int, ProgramState) means " -> s -> (a,s) " to end of type declaration:
foo' :: Int -> String -> ProgramState -> (Int, ProgramState)
-}

-- Returning State means returning a function.

-- The partial function will get applied when foo called with third parameter (state).
-- Foo is function that takes an int, a string, a previous state; returns a tuple of (return value, new state).
-- Use runState to extract the inner function.

initialState = PS []
doSomething = runState (foo 2 "blah") initialState

doSomethingCooler = runState (do (PS currentStack) <- get
                                 let top = head currentStack
                                 returnedFromFoo <- foo top "myString"
                                 put (returnedFromFoo : currentStack)
                                 return top
                             ) initialState

-- functions chained via >>=
-- each line where we call a function "in the state monad" behaves as follows:
   -- arguments we give it on that line are applied, yielding a partially applied function.
   -- because each state monad functions yields a State function (that needs an argument of type s before it can yields it's result tuple).
-- >>= : all functions in do loop are passed the current state as final parameter, execute, and then, once they return,
   -- the next function in the chain gets called using the previously return state.
...
someOtherFunctionWeCouldWrite x = x

-- do loop collapsed into one function, of type State (e.g. of type s -> (a,s).
-- it takes an initial state, and returns a tuple.
-- To run it, we extract the "inner function" from it's State wrapping using runState, and call it on some initial state.
...
rollDie' s =
    get s >>= (\generator ->
        let (value, newGenerator) = randomR (1,100) generator
        in put s newGenerator >>= (\_ -> return value))
-}
es      = evalState rollDie (mkStdGen 0)
rs      = runState  rollDie (mkStdGen 0)
(r4,g4) = runState  rollDie (mkStdGen 0)
(r5,g5) = runState  rollDie g4
(r6,g6) = runState  rollDie g5
#+END_SRC
