...
- translation into standard algebraic vocabulary
  - independent of stack machines and has been justified/implemented in a more general setting
  - converts a typed functional language (here, Haskell) to the vocabulary of categories
- stack-oriented interpretation of that vocabularysetting.
...
{-
Conversely, turn a stack function into a regular function,
initializing the stack to contain a and (),
evaluating the contained stack operations, and discarding the final ()
-}
evalStackFun0 :: StackFun a b -> (a -> b)
evalStackFun0 (SF f) a = b where (b, ()) = f (a, ())

-- can also formulate evalStackFun in more general terms:
...
- require that stackFun preserves the algebraic structure of that vocabulary.
...
used for automatic differentiation [Elliott, 2018]. A benefit is that
we need only implement a few type class instances rather than
manipulate any syntactic representation.
-}
------------------------------------------------------------------------------
{-
...
instance Category (->) where
  ...
  g ... f = g . f
{-
The corresponding structure preservation (homomorphism) properties are:
  id = stackFun id
...
The id equation is easy to satisfy, since it is already in solved form,
so we can use it directly as an implementation.
Instead, simplify the equation as follows:
-}

simplifyId :: [Test]
simplifyId  = U.tt "simplifyId"
  [ evalStackFun (stackFun id)    ()
  -- { definition of stackFun }
  , evalStackFun (SF (first id))  ()
...
simplifyLHS  = U.tt "simplifyLHS"
  [ evalStackFun (stackFun g ... stackFun f)     ()
  -- { definition of stackFun }
  , evalStackFun (SF (first g) ... SF (first f)) ()
  ]
  (evalStackFun (SF (first g) ... SF (first f)) ())
 where
  f x = x
...
simplifyRHS  = U.tt "simplifyRHS"
  [ evalStackFun (stackFun (g ... f))   ()
  -- { definition of stackFun }
  , evalStackFun (SF (first (g ... f))) ()
  -- { property of first and (...) }
  ]
  (evalStackFun (SF (first g ... first f)) ())
 where
  f x = x
  g x = x

-- The simplified specification:
simplifiedSpecification :: [Test]
simplifiedSpecification  = U.t "simplifiedSpecification"
  (evalStackFun (SF (first g) ... SF (first f))   ())
  (evalStackFun (SF (first g ... first f))        ())
 where
  f x = x
...
-- Strengthen this equation by generalizing from first g and first f
-- to arbitrary functions
-- (also called “g” and “f ” and having the same types as first g and first f)
strengthenedSpec :: [Test]
strengthenedSpec  = U.t "strengthenedSpec"
  (evalStackFun (SF g ... SF f) ())
  (evalStackFun (SF (g ... f))  ())
 where
  f x = x
  g x = x
...
-- use it and simplified id to define both category operations
...
So far, above is only sequential, so the stackk is not used.
...
class MonoidalP k where
  ...
  -- two forms that are sometimes more convenient
  -- (one of which we’ve already seen in a more specialized context):
...
secondInTermsOfFirst  = U.t "secondInTermsOfFirst"
  ( second g                    (1,2) )
...
firstInTermsOfSecond  = U.t "firstInTermsOfSecond"
  ( first f                      (1,2) )
  ( (swap ... second f ... swap) (1,2) )
 where
  f a = a

-- Thanks to these relationships, any two of (×), first, and second
-- can be defined in terms of the other.
-- Here it will be convenient to calculate a definition of first on StackFun,
...
Our required homomorphism equation for first is thus equivalent to the following
...
we can use it as a definition, expressing second and (×) in terms of it:
-}
...
We can get left-to-right evaluation by reformulating parallel composition as
...
The projections exl and exr arise from translation of pattern-matching on pairs.
Duplication is used for translation of pair formation and application expressions,
in the guise of the “fork” operation [Elliott, 2017, Section 3]:
-}
...
Dual of  MonoidalP and Cartesian for products work on coproducts (sums)
-}
class MonoidalS k where
  (.+.) :: (a `k` c) -> (b `k` d) -> (Either a b `k` Either c d)

instance MonoidalS (->) where
  f .+. g = \case Left a -> Left (f a); Right b -> Right (g b)
...
instance Distributive (->) where
  ...
  undistr = \case Left (u, b) -> (Left u, b); Right (v, b) -> (Right v, b)
...
-- With the MonoidalS and Distributive instances for (→),
-- we can define a correct MonoidalS instance for StackFun:
