...
(**
/Applications/CoqIdE_8.4.app/Contents/Resources/bin/coqc x02Induction.v
*)


Require Export x02Induction.
...
Proof. reflexivity. Qed.
Eval simpl in count 4 (remove_one 5 [2;1;4;5;1;4]).
...
Proof.
  ...
  Case "l = []".     (* pred (length []) = length (tl []) *)
    simpl.           (*                0 = 0                *)
    reflexivity.
  Case "l = n::l'".  (* pred (length (n :: l')) = length (tl (n :: l')) *)
...
(** Dont just read example proofs.
    Important to understand details of each one. *)
...
(** First prove a lemma about the equation about [snoc] that would have
    enabled us to make progress : *)
...
(** In more compressed style, the above proof might look more like this: *)
...
(** Write/prove theorem about bags involving [count] and [sum].*)
(* HC/TODO *)
