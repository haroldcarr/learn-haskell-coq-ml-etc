...
VEC n = record
  ...
  ; F-map->~>   = \ f g -> extensionality \ xs -> xxx g f xs

  }
 where
  -- all this does is rearrange things to solve "impedance" mismatch
  xxx : {T S R : Set}{n : Nat} -> (g : S → T) -> (f : R → S) -> (xs : Vec R n)
      -> vMap (λ x → g (f x)) xs == vMap g (vMap f xs)
  xxx g f xs
    rewrite sym (vMapCpFact (λ x → refl (g (f x))) xs)
    = refl (vMap g (vMap f xs))
